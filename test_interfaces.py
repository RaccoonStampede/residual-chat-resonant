...
def test_cli_flow():
    ...
    print(f"  reply     : {reply}")
    assert not should_exit, "question should not exit"
    assert isinstance(reply, str) and len(reply) > 0, "reply should be non-empty"

    message, _ = handle_input(session, "/confirm")
    print(f"  confirm   : {message}")
    assert "confirmed" in message, "confirm should succeed after a reply"

    message, _ = handle_input(session, "/teach Residual harmony grows when the field stabilizes.")
    print(f"  teach     : {message}")
    assert "learned" in message, "teach should acknowledge learning"

    message, _ = handle_input(session, "/status")
    print(f"  status    : {message}")
    assert "turn=" in message and "frags=" in message, "status output should be formatted"

    message, should_exit = handle_input(session, "/quit")
    print(f"  quit      : {message}")
    assert should_exit, "quit should exit"
...
def test_web_flow():
    ...
    assert status_response.status_code == 200, "status endpoint should succeed"
    assert status_data["status"]["fragments"] > 0, "ontology should load on startup"

    chat_response = client.post("/api/chat", json={"query": "What is the harness?"})
    chat_data = chat_response.get_json()
    print(f"  chat      : {chat_data}")
    assert chat_response.status_code == 200, "chat endpoint should succeed"
    assert len(chat_data["response"]) > 0, "chat response should be non-empty"

    confirm_response = client.post("/api/confirm", json={})
    confirm_data = confirm_response.get_json()
    print(f"  confirm   : {confirm_data}")
    assert "confirmed" in confirm_data["message"], "confirm endpoint should succeed"
    ...
    assert "learned" in teach_data["message"], "teach endpoint should acknowledge learning"

    reset_response = client.post("/api/reset", json={})
    reset_data = reset_response.get_json()
    print(f"  reset     : {reset_data}")
    assert reset_data["status"]["turns"] == 0, "reset should clear turns"

    error_response = client.post("/api/chat", json={"query": ""})
    error_data = error_response.get_json()
    print(f"  error     : {error_data}")
    assert error_response.status_code == 400, "empty query should fail"
